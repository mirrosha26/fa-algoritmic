...
class Height_value(Sensor_value):
    def __init__(self, parameter, height):
            super().__init__(parameter)
            self.height = height

    def __str__(self):
        return f'{self.parameter} на высоте {self.height} м. [{self.date_time}]'     
...
class HeightHygrometer(Hygrometer):
    ClassName = "Hygrometer"
    ...
    def get(self):
        result = self.signal() * (self.max - self.min) + self.min
        self.exit = Height_value(result, self.height())
        return self.exit


    def height(self):
        self.h = round((1/(random.uniform(0, 1)) -1),2)
        return self.h
...
if __name__ == "__main__":
    ...
    height_hygrometer_1 = HeightHygrometer('Высотный датчик C')
